...
#define CKLINK_REG8_SET_IDLE_DELAY(cklink_reg8, idle_delay)                                                            \
    (((cklink_reg8)&0xf07ffff8) | (((idle_delay)&0x7)) | ((((idle_delay) >> 3) & 0x1f) << 23))
...
#define CKLINK_REG8_SET_PROGBUF_SIZE(cklink_reg8, progbuf_size)                                                        \
    (((cklink_reg8)&0xfffe0fff) | (((progbuf_size)&0x1f) << 12))
...
#define CKLINK_REG8_SET_ABSTRACTAUTO(cklink_reg8, abstractauto)                                                        \
    (((cklink_reg8)&0xffdfffff) | (((abstractauto)&0x1) << 21))
...
*/
// CKLINK_API 
// int link_write_tms(struct cklink *link, uint32_t value, int nbit);
int link_write_tms(void *handle, uint32_t value, int nbit);
